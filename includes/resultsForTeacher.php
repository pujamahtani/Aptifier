...
$results=$test->getResultsForTeacher($user_id);
	$display=1;
}
if(isset($_POST['excel'])){
	$file = fopen("sample csv files/ResultsTeacher.csv","w");
	$results=$test->getResultsForTeacher($user_id);
	foreach ($results as $row)
  {
	...
	//fputcsv needs an source to put a file thus array you can also append it to a string and directl send it. i did it if i need to send tht data that is being saved to csv file to some other function
		 $class="D".$test_class_id.$test_division;
		$name=$user_first_name." ".$user_last_name;;
	$list=array("$test_name,$class,$test_date,$score,$name,$test_level");
...
-->
  ...
  <?php
    ...
    <td><?php echo "D".$test_class_id.$test_division; ?></td> 
    <td><?php echo $level; ?></td>
    <td style="width:20%;"><?php echo $test_date; ?></td>
    <td align="center"><?php echo $score; ?></td>
    <td style="width:30%;"><u><?php echo $user_first_name." ".$user_last_name; ?></u></td>
  </tr>
  	<?php
	  }
	  ?>
